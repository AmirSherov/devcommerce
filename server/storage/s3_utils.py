...
class S3StorageManager:
    ...
    def _get_s3_key(self, file_path: str, container_name: str = None) -> str:
        """Генерирует S3 ключ для файла"""
        if container_name:
            return f"storage/{container_name}/{file_path}"
        return f"storage/{file_path}"
    
    def upload_file(self, file_obj, file_name: str, container_name: str = None, 
        ...
        try:
            ...
            # Генерируем S3 ключ
            s3_key = self._get_s3_key(file_name, container_name)
            ...
            logger.error(f"Ошибка загрузки файла в S3: {error_code} - {e}")
    ...
    def delete_file(self, s3_key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            error_code = e.response['Error']['Code']
            logger.error(f"Ошибка удаления файла из S3: {error_code} - {e}")
            return {"success": False, "error": f"S3 ошибка: {error_code}"}
        except Exception as e:
    ...
    def file_exists(self, s3_key: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Неожиданная ошибка при проверке файла: {e}")
            return False
    ...
    def get_file_info(self, s3_key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            error_code = e.response['Error']['Code']
            logger.error(f"Ошибка получения информации о файле: {error_code} - {e}")
            return {"success": False, "error": f"S3 ошибка: {error_code}"}
        except Exception as e:
    ...
    def list_files(self, prefix: str = "", max_keys: int = 1000) -> Dict[str, Any]:
        ...
        try:
            ...
            error_code = e.response['Error']['Code']
            logger.error(f"Ошибка получения списка файлов: {error_code} - {e}")
            return {"success": False, "error": f"S3 ошибка: {error_code}"}
        except Exception as e:
    ...
    def copy_file(self, source_key: str, destination_key: str) -> Dict[str, Any]:
        ...
        Args:
            source_key: Исходный S3 ключ
            destination_key: Целевой S3 ключ
        ...
        try:
            ...
            return {"success": True, "source_key": source_key, "destination_key": destination_key}
            
        except ClientError as e:
            error_code = e.response['Error']['Code']
            logger.error(f"Ошибка копирования файла в S3: {error_code} - {e}")
            return {"success": False, "error": f"S3 ошибка: {error_code}"}
        except Exception as e:
            logger.error(f"Неожиданная ошибка при копировании файла: {e}")
            return {"success": False, "error": str(e)}
    ...
    def get_bucket_size(self, prefix: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            error_code = e.response['Error']['Code']
            logger.error(f"Ошибка получения размера бакета: {error_code} - {e}")
            return {"success": False, "error": f"S3 ошибка: {error_code}"}
        except Exception as e:
            logger.error(f"Неожиданная ошибка при получении размера бакета: {e}")
            return {"success": False, "error": str(e)}
...
def upload_file_to_s3(file_obj, file_name: str, container_name: str = None, 
                      content_type: str = None) -> Dict[str, Any]:
    """Загружает файл в S3"""
    return s3_manager.upload_file(file_obj, file_name, container_name, content_type)
...
def get_file_url_from_s3(s3_key: str, expiration: int = 3600) -> str:
    """Получает URL для доступа к файлу"""
    return s3_manager.get_file_url(s3_key, expiration)
...
def get_file_info_from_s3(s3_key: str) -> Dict[str, Any]:
    """Получает информацию о файле в S3"""
    return s3_manager.get_file_info(s3_key)


def list_files_in_s3(prefix: str = "", max_keys: int = 1000) -> Dict[str, Any]:
    """Получает список файлов в S3"""
    return s3_manager.list_files(prefix, max_keys)


def copy_file_in_s3(source_key: str, destination_key: str) -> Dict[str, Any]:
    """Копирует файл в S3"""
    return s3_manager.copy_file(source_key, destination_key)


def get_bucket_size_from_s3(prefix: str = "") -> Dict[str, Any]:
    """Получает общий размер файлов в бакете"""
    return s3_manager.get_bucket_size(prefix) 