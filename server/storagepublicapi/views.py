...
class PublicAPIKeyView(APIView):
    ...
    def get(self, request):
        ...
        try:
            api_key = request.user  # В нашем случае это PublicAPIKey
            ...
            return Response({
                'success': False,
                'error': 'Ошибка получения информации об API ключе'
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    
    def post(self, request):
        """Создание нового API ключа для контейнера"""
        try:
            # Проверяем, что пользователь имеет доступ к API
            if request.user.container.user.plan == 'standard':
                return Response({
                    'success': False,
                    'error': 'API доступ доступен только для Premium и Pro планов',
            ...
            # Проверяем, что у контейнера еще нет API ключа
            container = request.user.container
            if hasattr(container, 'public_api_key'):
                return Response({
                    'success': False,
                    'error': 'API ключ для этого контейнера уже существует',
                    'error_code': 'API_KEY_EXISTS'
                }, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({
                'success': True,
                'api_key': PublicAPIKeySerializer(api_key).data,
...
class PublicFileListView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            serializer = PublicFileListSerializer(files_page, many=True)
...
class PublicFileDetailView(APIView):
    ...
    def get(self, request, file_id):
        ...
        try:
            ...
            serializer = PublicFileDetailSerializer(storage_file)
...
class PublicAPIStatsView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            today = timezone.now().date()
            today_stats = PublicAPIUsage.objects.filter(
                api_key=api_key,
                date=today
            ).first()
            
            # Общая статистика
            total_requests = PublicAPIRequest.objects.filter(api_key=api_key).count()
            ...
            # Статистика файлов
            total_files = StorageFile.objects.filter(container=container, is_active=True).count()
            files_uploaded = PublicAPIUsage.objects.filter(api_key=api_key).aggregate(
                total=Sum('files_uploaded')
            )['total'] or 0
            files_downloaded = PublicAPIUsage.objects.filter(api_key=api_key).aggregate(
                total=Sum('files_downloaded')
            )['total'] or 0
            
            # Размеры файлов
            total_upload_size = PublicAPIUsage.objects.filter(api_key=api_key).aggregate(
                total=Sum('total_upload_size')
            )['total'] or 0
            total_download_size = PublicAPIUsage.objects.filter(api_key=api_key).aggregate(
                total=Sum('total_download_size')
            )['total'] or 0
            
            # Производительность
            avg_response_time = PublicAPIUsage.objects.filter(api_key=api_key).aggregate(
                avg=Sum('average_response_time') / Count('id')
            )['avg'] or 0
            
            # Популярные эндпоинты
            popular_endpoints = PublicAPIRequest.objects.filter(api_key=api_key).values(
                'endpoint'
            ).annotate(count=Count('endpoint')).order_by('-count')[:5]
            ...
            requests_remaining = max(0, limits.requests_per_hour - recent_requests)
